...
class Command(BaseCommand):
    help: str = "Reset daily budgets and reactivate eligible campaigns"
    ...
    def handle(self, *args: Any, **options: Dict[str, Any]) -> None:
        ...
        if not dry_run:
            brands_updated: int = Brand.objects.update(daily_spend=Decimal("0"))
            campaigns_updated: int = Campaign.objects.update(daily_spend=Decimal("0"))
        else:
            brands_updated = Brand.objects.count()
            campaigns_updated = Campaign.objects.count()

        self.stdout.write(
            f"{'Would reset' if dry_run else 'Reset'} daily spend for "
        ...
        if not dry_run:
            self.stdout.write(self.style.SUCCESS("Daily budget reset completed successfully"))
        else:
